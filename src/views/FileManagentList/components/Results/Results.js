...
const Results = props => {
  const { className, page, size, order, orderBy, onRequestSort, onRequestPagination, isLoading, files, numberOfElements, totalPages, fetchFiles, ...rest } = props;

  const classes = useStyles();

  const [selectedFiles, setSelectedResults] = useState([]);

  const handleSelectAll = event => {
    const selectedFiles = event.target.checked
      ? files.map(customer => customer.id)
      : [];

    setSelectedResults(selectedFiles);
  };
  ...
  const handleSelectOne = (event, id) => {
    ...
    setSelectedResults(newSelectedFiles);
  };
  ...
  return (
    ...
    >
      <Card>
        <TableToolbar title="Todos as contas de usuário" numSelected={selectedFiles.length} />
        <Divider />
                    ...
                    >
                      <TableCell padding="checkbox">
                        <Checkbox
                          ...
                          color="primary"
                          onChange={event =>
                            handleSelectOne(event, file.id)
                          }
                          value={selectedFiles.indexOf(file.id) !== -1}
                        />
                      </TableCell>
                      <TableCell>
                        <div className={classes.nameCell}>
                          <Avatar
                            ...
                            src={file.avatar ? file.avatar.uri : ''}
                          >
                            {getInitials(file.name + ' ' + file.lastName)}
                          </Avatar>
                          <div>
                            {file.name}
                      ...
                      <TableCell>{file.enabled === true ? "Sim" : "Não"}</TableCell>
                      <TableCell>{file.locked === true ? "Sim" : "Não"}</TableCell>
                      <TableCell>
                        <div>
                          <div>{file.createdBy}</div>
                          <Moment format="DD/MM/YYYY HH:mm:ss" local>{file.createdDate}</Moment>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div>
                          <div>{file.lastModifiedBy}</div>
                          <Moment format="DD/MM/YYYY HH:mm:ss" local>{file.lastModifiedDate}</Moment>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div>
                          <Button
                            href={`localhost:8084/api/v1/file_/download?file=${file.name}.pdf`}
